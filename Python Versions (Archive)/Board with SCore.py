...
# ------------------- Character-cell scoreboard helpers --------------------
def make_char_cells(parent, n):
    """Create n fixed one-character boxes (each gray box = 1 char)."""
    cells = []
    for i in range(n):
        L = tk.Label(parent, text=" ", width=2, height=1,
                     font=("Consolas", 28, "bold"),
                     bg="#cfcfcf", fg="black", bd=2, relief="ridge")
        L.grid(row=0, column=i, padx=3, pady=3)
        cells.append(L)
    return cells

def write_cells(cells, text):
    t = (text or "")[:len(cells)]
    t = t + " " * (len(cells) - len(t))
    for L, ch in zip(cells, t):
        L.config(text=ch)

# Build the top blue scoreboard block
sb = tk.Frame(root, bg="#1f6fbf", bd=4, relief="ridge")
sb.pack(fill="x", padx=110, pady=(0, 8))

# Row 0: HOME [2] — PERIOD [2] — GUEST [2]
row0 = tk.Frame(sb, bg="#1f6fbf"); row0.pack(pady=6)

# --- HOME ---
left = tk.Frame(row0, bg="#1f6fbf"); left.pack(side="left", padx=12)
left_cells = tk.Frame(left, bg="#1f6fbf"); left_cells.pack()
home_score_cells = make_char_cells(left_cells, 2)

# --- PERIOD ---
center = tk.Frame(row0, bg="#1f6fbf"); center.pack(side="left", padx=40)
center_cells = tk.Frame(center, bg="#1f6fbf"); center_cells.pack()
period_cells = make_char_cells(center_cells, 2)

# --- GUEST ---
right = tk.Frame(row0, bg="#1f6fbf"); right.pack(side="left", padx=40)
right_cells = tk.Frame(right, bg="#1f6fbf"); right_cells.pack()
guest_score_cells = make_char_cells(right_cells, 2)

# Row 1 & 2: Penalties -> [PLYR 2 cells][MM 2 cells]  (seconds lamps are not characters)
def penalty_row(parent):
    wrap = tk.Frame(parent, bg="#1f6fbf"); wrap.pack(pady=4)
    row = tk.Frame(wrap, bg="#1f6fbf"); row.pack()
    ply = make_char_cells(row, 2)
    mm  = make_char_cells(row, 2)
    return {"ply": ply, "mm": mm}

lower = tk.Frame(sb, bg="#1f6fbf"); lower.pack(pady=2, fill="x")
left_pen  = tk.Frame(lower, bg="#1f6fbf");  left_pen.pack(side="left",  padx=12)
right_pen = tk.Frame(lower, bg="#1f6fbf"); right_pen.pack(side="right", padx=12)

home_pen1  = penalty_row(left_pen)
home_pen2  = penalty_row(left_pen)
guest_pen1 = penalty_row(right_pen)
guest_pen2 = penalty_row(right_pen)

def write_pen_cells(slot, player_txt, minutes_txt):
    write_cells(slot["ply"], (player_txt or "  "))
    write_cells(slot["mm"],  (minutes_txt or "  "))

def update_scoreboard():
    # Scores
    write_cells(home_score_cells, f"{app_state['home_score']:02d}"[-2:])
    write_cells(guest_score_cells, f"{app_state['guest_score']:02d}"[-2:])
    # Period
    write_cells(period_cells, f"{app_state['period']:>2}"[-2:])
    ...
    def team_slots(team):
        ...
        for i in range(2):
            if i < len(L):
                ...
                mm = f"{(secs // 60):02d}"
                slots.append((f"{p:02d}", mm))
            else:
                slots.append(("  ", "  "))

        # live preview for entry
        if app_state["mode"] == "PENALTY_ENTRY" and app_state["team"] == team:
            idx = len(L)
            if idx < 2:
                if app_state["stage"] == "PLAYER":
                    slots[idx] = (mask2(app_state["buffer"]), "  ")
                elif app_state["stage"] == "TIME":
                    p_txt = f"{int(app_state['_player']):02d}" if app_state["_player"] is not None else "  "
                    if app_state.get("time_default", False):
                        mm_txt = "02"
                    else:
                        mm_txt = mask_time(app_state["buffer"])[:2].replace(":", "")
                    slots[idx] = (p_txt, mm_txt)

        # live preview for edit (minutes only)
        if app_state["mode"] == "PENALTY_EDIT" and app_state["team"] == team:
            i = app_state["edit_index"]
            if i < len(L) and i < 2 and app_state["stage"] == "TIME":
                p_txt = f"{L[i]['player']:02d}"
                mm_txt = "02" if app_state.get("time_default", False) else mask_time(app_state["buffer"])[:2].replace(":", "")
                slots[i] = (p_txt, mm_txt)

        return slots

    h1, h2 = team_slots("HOME")
    g1, g2 = team_slots("GUEST")
    write_pen_cells(home_pen1,  *h1)
    write_pen_cells(home_pen2,  *h2)
    write_pen_cells(guest_pen1, *g1)
    write_pen_cells(guest_pen2, *g2)

# ------------------- LCD (now also triggers board refresh) --------------------
def lcd_scores():
    status_var.set(
...
def period_plus_one():
    app_state["period"] = (app_state["period"] % 9) + 1
    update_scoreboard()
...
# ------------------- Overlay dispatcher (placeholder) ------
...
make_btn(frame, "PERIOD\n+1", 3, 5, color="white", command=period_plus_one)
make_btn(frame, "", 3, 6)
make_btn(frame, "", 3, 9)
